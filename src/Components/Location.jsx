...
import {addLocation,removeLocation,clearLocation,setServicesProgress} from '../redux/service/service-actions';
import {setUserServiceStatus} from '../redux/user/user-actions';
import {Snackbar,CircularProgress,Backdrop,withStyles,TableContainer,Paper,Table,TableHead,TableBody,TableCell,TableRow,TextField,Grid,makeStyles,Button,Typography} from '@material-ui/core';
...
const {currentUser,setUserStatus,addLocation,clearLocation,setProgress} = props;
  ...
  function editLocation(location){
    console.log(detailsLength);
      var address = location.Address.slice(detailsLength+1,location.Address.length);
    setData({
  ...
  async function saveToDatabase(){
    ...
    if(result){
      if(result.data.responseCode === 200){
        ...
        setLoading(false);
        setSeverity('error');
        setAlert('Unexpected error occured ! Saving locations was unsuccessfull.');
        setOpen(true);
      }
    }
  }
...
useEffect(()=>{
  if(currentUser){
   ...
   gettingLocations(request)
   .then(res => addingSavedLocations(res));
...
const classes = useStyles();
  return (
   ...
   {props.locationList.length === 0? <Typography variant='h5' style={{textAlign:'center',marginTop:'50px',color:'#bdbdbd'}}><WarningIcon/>  No locations added yet!</Typography> :
   <div style={{width:'100%',textAlign:'right'}}>
    <Button className={classes.button} style={{width:'22%',margin:'5px'}}  variant="contained" onClick={()=>{saveToDatabase();}} startIcon={<SaveAltIcon/>}>
       Apply & Save
     </Button>
     </div>
   }
